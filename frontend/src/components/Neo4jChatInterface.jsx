...
const RAG_API_BASE_URL =
    "https://ragapi-service-722252932298.us-central1.run.app"
...
function Neo4jChatInterface({repoId}) {
    ...
    const entryNotification = () => {
        addNotification({
            ...
            message:
                "Your files have been processed. You can now analyze your code.",
            duration: 10000,
            native: true,
        })
    }
    ...
    const fetchFiles = async () => {
        ...
        try {
            ...
            const response = await axios.get(
                `${RAG_API_BASE_URL}/api/graph/files?repo_id=${repoId}`
            )
        ...
        try {
            ...
            try {
                response = await axios.get(
                    `${RAG_API_BASE_URL}/api/graph/file-data?repo_id=${repoId}&file_path=${file.path}`
                )
                if (response.data.success) {
                ...
                console.log(
                    `First file-data URL format failed, trying alternative...${error}`
                )
                try {
                    response = await axios.get(
                        `${RAG_API_BASE_URL}/graph/file-data?repo_id=${repoId}&file_path=${file.path}`
                    )
                    if (response.data.success) {
                        setFileData(response.data.fileData)
                    }
                    ...
                    console.error(
                        "Error fetching file data with alternative URL:",
                        innerError
                    )
                }
            } finally {
            ...
            try {
                graphResponse = await axios.get(
                    `${RAG_API_BASE_URL}/api/graph/file-graph?repo_id=${repoId}&file_path=${file.path}`
                )
                if (graphResponse.data.success) {
                    ...
                    const filteredNodes = graphResponse.data.graphData.nodes
                        ? graphResponse.data.graphData.nodes.filter((node) =>
                ...
                console.log(
                    `First file-graph URL format failed, trying alternative...${error}`
                )
                try {
                    graphResponse = await axios.get(
                        `${RAG_API_BASE_URL}/graph/file-graph?repo_id=${repoId}&file_path=${file.path}`
                    )
                    if (graphResponse.data.success) {
                        // Filter out isolated nodes (nodes without any connections)
                        const connectedNodeIds = new Set()
                        if (graphResponse.data.graphData.links) {
                            graphResponse.data.graphData.links.forEach(
                                (link) => {
                                    connectedNodeIds.add(link.source)
                                    connectedNodeIds.add(link.target)
                                }
                        ...
                        const filteredNodes = graphResponse.data.graphData.nodes
                            ? graphResponse.data.graphData.nodes.filter(
                    ...
                    console.error(
                        "Error fetching graph data with alternative URL:",
                        innerError
                    )
                }
            } finally {
    ...
    const handleChatQuerySubmit = async (e) => {
        ...
        try {
            ...
            if (selectedFile.isRepoContext) {
                ...
                requestData.file_path = selectedFile.name
    ...
    const handleGraphFilterChange = (filterType, filterValue, isEnabled) => {
        setGraphFilters((prevFilters) => {
            ...
            if (filterType === "nodeType") {
                ...
                newFilters.showAllNodeTypes =
                    newEnabledNodeTypes.size === newFilters.nodeTypes.length
            } else if (filterType === "relationshipType") {
                const newEnabledRelationshipTypes = new Set(
                    newFilters.enabledRelationshipTypes
                )
                ...
                newFilters.enabledRelationshipTypes =
                    newEnabledRelationshipTypes
                newFilters.showAllRelationshipTypes =
                    newEnabledRelationshipTypes.size ===
                    newFilters.relationshipTypes.length
    ...
    const filteredGraphData = React.useMemo(() => {
        if (
            !graphFilters.enabledNodeTypes ||
            !graphFilters.enabledRelationshipTypes
        ) {
            return graphData
        }
    ...
    return (
        <div className="analyo-interface">
            ...
            {isMobile && (
                <button
                    className={`mobile-menu-toggle ${theme}-theme`}
            ...
            <PanelGroup direction="horizontal">
                ...
                >
                    <aside className={`left-sidebar ${theme}-theme`}>
                        ...
                        {isMobile && (
                            <button
                                className={`sidebar-close-btn ${theme}-theme`}
                                onClick={closeMobileSidebar}
                            ...
                            <div className="file-tree-content">
                                <FileList
                                    ...
                                    onFileSelect={(file) => {
                                        ...
                                        if (isMobile) {
                                            setIsMobileSidebarOpen(false) // Auto-close on mobile after file selection
                                        }
                                    }}
                ...
                {!isMobile && (
                    <PanelResizeHandle
                        className={`resize-handle ${theme}-theme`}
                    />
                )}
                ...
                <Panel minSize={50} className={`messages-panel ${theme}-theme`}>
                    <main className={`main-content ${theme}-theme`}>
                        <div className={`chat-container ${theme}-theme`}>
                            {/* View Graph Button */}
                            <button
                                className={`view-graph-btn ${theme}-theme ${
                                    isGraphViewOpen ? "active" : ""
                                }`}
                                onClick={() =>
                            ...
                            <div
                                className={`chat-input-section ${theme}-theme`}
                            >
                                <form
                                ...
                                >
                                    <div
                                        className={`input-container ${theme}-theme`}
                                    >
                                        <textarea
                ...
                <div className="graph-overlay">
                    <div className="graph-panel">
                        <div className="graph-header">
                            ...
                            <button
                                className={`close-graph-btn ${theme}-theme`}
                        ...
                        <div className="graph-content">
                            ...
                            {/* Mobile Layout - Vertical Resizable Panels */}
                            <PanelGroup
                                direction="vertical"
                                className="graph-panel-group"
                            >
                                <Panel
                                ...
                                <PanelResizeHandle
                                    className={`graph-resize-handle ${theme}-theme`}
                                />
